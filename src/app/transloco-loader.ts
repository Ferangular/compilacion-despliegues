...
export class TranslocoHttpLoader implements TranslocoLoader {
  ...
  getTranslation(lang: string): Observable<Translation> {
    ...
    if (this.loadingSubjects.has(lang)) {
      ...
      return this.loadingSubjects
        .get(lang)!
        .asObservable()
        .pipe(map((translation) => translation || ({} as Translation)));
    }
    ...
    return subject.asObservable().pipe(map((translation) => translation || ({} as Translation)));
  }

  // Preload translations for better performance
  async preloadTranslations(languages: string[]): Promise<void> {
    const promises = languages.map((lang) => firstValueFrom(this.getTranslation(lang)));
